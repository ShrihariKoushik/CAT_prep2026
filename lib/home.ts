// Home-screen data assembly. Also the retry-on-page-load path:
// missing/failed sets get an instant seed-bank fallback, and real generation
// is retried in the background (through the GenerationJob lock) via after().
...
export type SessionCardData = {
  ...
  expiresAt: number; // IST midnight, epoch ms
  setId: string | null;
  isFallback: boolean;
  started: boolean; // attempt exists but not submitted → card says "Continue"
  score: number | null;
...
export async function getHomeData() {
  ...
  const state = await replenishFreeze(today).then(() => getUserState());

  const sessions: SessionCardData[] = [];
  for (const section of SECTIONS) {
    const slot = SLOT_FOR_SECTION[section];
    let set = await prisma.questionSet.findUnique({
      where: { day_section: { day: today, section } },
      include: { attempt: true },
    });

    // Missing (cron never ran / failed silently) → instant fallback + background retry
    if (!set) {
      await createFallbackSet(today, section);
      set = await prisma.questionSet.findUnique({
        where: { day_section: { day: today, section } },
        include: { attempt: true },
      });
      after(() => generateWithLock(today, section).catch(() => {}));
  ...
  const logs = await prisma.dayLog.findMany({ where: { day: { gte: since } } });
  ...
  return {
    ...
    heatmap,
    redoDue: await dueReviewCount(today),
  };
}
